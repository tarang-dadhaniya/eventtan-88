...
@Component({
  selector: "app-add-speakers-modal",
  standalone: true,
      ...
      >
        ...
        <form class="flex-1 flex flex-col min-h-0" (ngSubmit)="onSubmit()">
          ...
          <div class="flex-1 overflow-y-auto px-[25px] py-6 min-h-0">
            ...
            <!-- Social Media and Speakers For Row -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <!-- Social Media -->
              <div class="relative">
                <label class="block text-base font-medium text-[#212529] mb-2">
...
export class AddSpeakersModalComponent {
  @Input() isOpen = false;
  @Input() editMode = false;
  @Input() set speakerData(data: any) {
    if (data) {
      this.formData = {
